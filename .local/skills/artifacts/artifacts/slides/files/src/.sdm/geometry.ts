...
export function resizeHeightAnchored(
  frame: Frame,
  height: number,
  anchor: HeightAnchor,
  rotationDeg = 0,
): Frame {
  const delta = height - frame.height;
  if (delta === 0) {
    return frame;
  }
  const rotationRad = (rotationDeg * Math.PI) / 180;
  const centerShift = (anchor === 'top' ? delta : -delta) / 2;
  const centerX =
    frame.x + frame.width / 2 - centerShift * Math.sin(rotationRad);
  const centerY =
    frame.y + frame.height / 2 + centerShift * Math.cos(rotationRad);

  return {
    x: Math.round(centerX - frame.width / 2),
    y: Math.round(centerY - height / 2),
    width: frame.width,
    height: Math.round(height),
  };
