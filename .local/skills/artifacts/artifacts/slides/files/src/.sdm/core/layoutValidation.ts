...
interface TextOwner {
  ...
  /** Whether the renderer hides lines outside the frame. */
  clipsBody: boolean;
}
  ...
  for (const element of elements) {
    ...
    if (element.type === 'text') {
      owners.push({
        ...
        transformed,
        clip,
        clipsBody: element.body.overflow === 'clip',
      });
  ...
  table.rows.forEach((row, rowIndex) => {
    ...
    row.cells.forEach((cell, cellIndex) => {
      ...
      owners.push({
        ...
        transformed,
        clip,
        // Table cells clip at the cell wrapper regardless of body.overflow.
        clipsBody: true,
      });
      columnIndex += colSpan;
    });
...
function layoutOwner(owner: TextOwner): OwnerLayout {
  ...
  const visibleLines = owner.clipsBody
...
function layoutText(body: TextBody, frame: Frame): TextLayout {
  ...
  // Negative slack matches flex alignment: an overflowing middle or bottom
  // body spills upward as well as, or instead of, downward.
  const verticalSlack = availableHeight - cursorY;
