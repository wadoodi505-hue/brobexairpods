...
Real-time behavior (physics, animations, collisions, game loops) happens too fast for an agent to observe directly. By the time you take a screenshot, the action is over. This reference uses ECS time-control tools to freeze, step, and diff state frame by frame.

Use the user's request to identify the behavior to debug.
