...
When `requestSecrets` is available, use it to collect chat-owned secrets, shared by the base environment and all attached code-execution environments. Conversation secrets are not injected automatically. Use `replit secrets list` to discover key names and `replit secrets exec --keys API_KEY -- command` to pass only the required secrets to a command.

Do not use `requestEnvVars`, `setEnvVars`, or `deleteEnvVars` in a conversation. After the conversation becomes a project, all environment-variable callbacks are available.
